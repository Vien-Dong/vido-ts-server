$(document).ready(function () {
  ...
  function spinWheel(record_id) {
    ...
    if (chance < 0.8) {
      ...
      do {
        ...
        (targetAngle >= 23.5 && targetAngle <= 66.5) ||
        (targetAngle >= 112.5 && targetAngle <= 147.5)
      ); // Nếu rơi vào vùng cấm thì random lại
  ...
  function getPosition(position, record_id) {
    const rewards = [
      {
        min: 0,
        max: 22.5,
        text: "CHÚC MỪNG BẠN TRÚNG ĐƯỢC MỘT CHIẾC VÉ MAY MẮN LẦN SAU",
      },
      ...
      { min: 245.5, max: 291.5, text: "CHÚC MỪNG BẠN TRÚNG ĐƯỢC MỘT TÚI MÙ" },
      {
        min: 292.5,
        max: 336.5,
        text: "CHÚC MỪNG BẠN TRÚNG ĐƯỢC MỘT CHIẾC MÓC KHÓA",
      },
      {
        min: 337.5,
        max: 360,
    ...
    let rewardText =
      rewards.find((r) => position >= r.min && position <= r.max)?.text ||
      "XUI QUÁ MỘT CHÚT NỮA LÀ TRÚNG RỒI 🤡";
    $(".congratulation__note").text(rewardText);

    if (
      (position >= 148.5 && position <= 201.5) ||
      ...
      $(".congratulation__description").text(
        "Vui lòng đến gian hàng Cao đẳng Viễn Đông để nhận quà hoặc copy mã trúng thưởng này gửi fanpage Tuyển sinh Cao đẳng Viễn Đông"
      );

      axios
        .put(`/api/crm/update-cptarget?record_id=${record_id}`, {
          winning_code: code,
        })
        .catch(() => {
  ...
  $(document).on(
    ...
    function (event) {
      ...
      console.log(selectedIndustry); // In ra giá trị đã chọn
