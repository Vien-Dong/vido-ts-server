$(document).ready(function () {
    ...
    $(document).on("submit", ".ladi-form", function (event) {
        event.preventDefault();

        var inputNameValue = $('input[name="name"]').val();
        var selectedIndustry = $('select[name="Training_industry"]').val(); // Lấy ngành học đã chọn
        ...
        axios.post("/api/crm/create-cptarget", postData)
            .then((result) => {
                if (result.data && result.data.success) {
                    ...
                    $(".ladi-form")[0].reset(); // Reset form sau khi gửi thành công
                }
            })
            .catch((error) => {
